...
import { REMOTE_DEMO_IMAGES } from "@/lib/remoteImageUrls";
import {
  IMAGE_BLUR_DATA_URL,
...
export default function HomeHeroFeatured({ story, headingId }: HomeHeroFeaturedProps) {
  ...
  const resolved = resolveContentImageSrc(story.titleImageUrl);
  /** Real hero when the CMS has a photo; otherwise a shared editorial fallback (not the SVG placeholder). */
  const heroSrc = isBundledPlaceholderSrc(resolved) ? REMOTE_DEMO_IMAGES.portrait : resolved;
  const deck = story.summary?.trim() || "Read the full story and explore more on Culturin.";

  return (
    <Link
      href={href}
      className="group relative flex min-h-[min(48vh,26rem)] w-full flex-col justify-start overflow-hidden rounded-2xl border border-white/10 bg-[#0a1628] p-8 text-left shadow-[0_28px_90px_-28px_rgba(0,0,0,0.35)] outline-none transition-[transform,box-shadow] after:pointer-events-none after:absolute after:inset-0 after:rounded-2xl after:ring-1 after:ring-inset after:ring-white/10 after:content-[''] focus-visible:ring-2 focus-visible:ring-amber-400/80 sm:min-h-[min(52vh,30rem)] sm:rounded-3xl sm:p-10 lg:min-h-[32rem] lg:max-h-[40rem] dark:shadow-[0_24px_80px_-24px_rgba(0,0,0,0.9)]"
    >
      <Image
        src={heroSrc}
        alt=""
        ...
        className="object-cover opacity-55 transition duration-500 group-hover:scale-[1.02] group-hover:opacity-60"
        sizes="(max-width: 1024px) 100vw, 70vw"
        unoptimized={isBundledPlaceholderSrc(heroSrc)}
        aria-hidden
      />
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-[#0a1628]/80 via-[#0d2137]/75 to-[#0a1628]/88 dark:from-[#050a12]/85 dark:via-[#0a1628]/70 dark:to-[#020508]/90"
        aria-hidden
      />
      <div
      ...
      <div className="relative z-10 flex flex-1 flex-col">
        ...
        <div className="mt-8 max-w-2xl pb-2">
          <h1
            id={headingId}
