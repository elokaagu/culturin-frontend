...
import { StudioMediaGalleryPicker } from "./StudioMediaGalleryPicker";
...
const actionButtonClass =
  "inline-flex h-8 items-center rounded-full border border-[color:var(--c-rule)] bg-[color:color-mix(in_srgb,var(--c-bg)_40%,white)] px-3 text-xs font-medium text-[color:var(--c-ink)] transition hover:border-[color:var(--c-accent)] hover:bg-[color:color-mix(in_srgb,var(--c-accent)_12%,transparent)] disabled:cursor-not-allowed disabled:opacity-60 dark:bg-white/[0.04]";

function sanitizeFileName(name: string): string {
  return name.replace(/[^\w.+-]+/g, "-").replace(/^-+|-+$/g, "") || "upload.bin";
}

export function StudioImageUploadButton({
  onUploaded,
  buttonLabel = "Upload image",
  allowGalleryPick = true,
}: StudioImageUploadButtonProps) {
  ...
  return (
    <div className="flex flex-col items-start gap-1.5">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <button type="button" disabled={disabled} onClick={() => inputRef.current?.click()} className={actionButtonClass}>
          {uploading ? "Uploading..." : buttonLabel}
        </button>
        {allowGalleryPick ? (
          <button type="button" onClick={() => setPickerOpen(true)} className={actionButtonClass}>
            Choose from gallery
          </button>
        ) : null}
      </div>
      ...
      {allowGalleryPick ? (
        <StudioMediaGalleryPicker
          open={pickerOpen}
          onClose={() => setPickerOpen(false)}
          onSelect={(src) => {
            onUploaded(src);
            setMessage("Selected from gallery.");
          }}
        />
      ) : null}
    </div>
